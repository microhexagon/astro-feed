...
const formatDate = (date: Date, isLaterSection = false): string => {
  if (isLaterSection) {
...
export default function Launches() {
  ...
  const groupedLaunches: GroupedLaunches = useMemo(() => {
    ...
    launches.forEach((launch) => {
      ...
      // Check if launch is today, tomorrow, or later (including future times today)
      ...
      // Skip past launches (launchDate < new Date())
    });
    ...
    // Debug logging
    console.log('Launch counts - Today:', today.length, 'Tomorrow:', tomorrow.length, 'Later candidates:', laterCandidates.length);
    console.log('Tomorrow launches:', tomorrow.map((l: Launch) => ({ name: l.name, date: l.window_start })));

    return { today, tomorrow: tomorrowLimited, later };
  }, [launches]);

  const renderLaunchCard = (launch: Launch, isLaterSection = false) => {
